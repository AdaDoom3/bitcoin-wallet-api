...
function registerRoutes(application, getDB) {
  ...
  for(var i = 0, length = config.routes.length; i < length; i++) {
    ...
    registerRoute(application, controller, route, method, action, getDB);
  }
...
function registerRoute(application, controller, route, method, action, getDB) {
  application.route(route)[method](function(req, res, next) {
    controller[action](req, res, next, getDB());
  });
}
